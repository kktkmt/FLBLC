...
class Requester:
    ...
    def init_task(self, deposit, model_uri, num_rounds, k):
        contract_instance = self.w3.eth.contract(abi=self.truffle_file['abi'], address=self.contract_address)

        tx = contract_instance.functions.initializeTask(model_uri, num_rounds, k).buildTransaction({
    ...
    def calc_overall_scores(self, score_matrix, num_workers):
        ...
        for i in range(num_workers):
            overall_scores.append(
                min(m_scaled[i], (d[i] / max_d)))  # compute overall score as the minimum between m_scaled and d_scaled
    ...
    def compute_top_k(self, addresses, scores):
        ...
        while len(temp_scores) > 0:
            ...
            top_k_addresses.append(temp_addresses[index])
            top_k_scores.append(temp_scores[index])
            temp_scores = np.delete(temp_scores, index)
            temp_addresses = np.delete(temp_addresses, index)
        print(top_k_addresses, top_k_scores)
        return top_k_addresses, top_k_scores

    def reverse_auction(self, overall_scores, overall_addresses, worker_bid_dict):
        #     根据overall_addresses， 依次取出对应的worker的price和score然后调用contract_instance的reverseAuction方法
        data = []
        self.contract_instance = self.w3.eth.contract(abi=self.truffle_file['abi'], address=self.contract_address)
        for i in range(len(overall_addresses)):
            address = overall_addresses[i]
            score = overall_scores[i]
            bid = worker_bid_dict[address]
            # data.append({"workerAddress": address, "score": score, "bid": bid})
            data.append((address, score, bid))

        tx = self.contract_instance.functions.reverseAuction(data).buildTransaction({
            "gasPrice": self.w3.eth.gas_price,
            "chainId": 1337,
            "from": self.account.address,
            'nonce': self.w3.eth.getTransactionCount(self.account.address)
    ...
    def getWorkerScores(self):
        self.contract_instance = self.w3.eth.contract(abi=self.truffle_file['abi'], address=self.contract_address)

        workerScores = self.contract_instance.functions.getWorkerScores().call()
        return workerScores


    def getCommitteeScores(self):
        self.contract_instance = self.w3.eth.contract(abi=self.truffle_file['abi'], address=self.contract_address)

        committeeScores = self.contract_instance.functions.getCommitteeScores().call()
        return committeeScores


    def verifyRound(self, hashByte):
        self.contract_instance = self.w3.eth.contract(abi=self.truffle_file['abi'], address=self.contract_address)

        result = self.contract_instance.functions.verifyHash(hashByte).call()
        print(result)
        return result
